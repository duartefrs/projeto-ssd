...
def exibir_10_maiores_marcadores():
    # Passo 1: Carregar os dados de transferências (incluindo gols e clube)
    dados_transferencias = pd.read_csv('transferencias_jogadores.csv')

    # Passo 2: Carregar o arquivo Excel (dados de clubes e países)
    dados_cruzamento = pd.read_excel('cruzamento.xlsx')

    # Passo 3: Limpeza e preparação dos dados
    # Vamos fazer a junção entre os dados de transferências e países
    dados_agrupados = pd.merge(dados_transferencias, dados_cruzamento, on='team', how='inner')

    # Passo 4: Selecionar os 10 jogadores com o maior número de gols
    top_10_marcadores = dados_agrupados.nlargest(10, 'goals')

    # Passo 5: Exibir a tabela com os 10 maiores marcadores
    st.write("Tabela com os 10 Maiores Marcadores de Gols:")
    st.write(top_10_marcadores[['name', 'goals', 'team', 'Country']])

    # Passo 6: Criar gráfico de barras para os 10 maiores marcadores
    plt.figure(figsize=(12, 8))
    plt.barh(top_10_marcadores['name'], top_10_marcadores['goals'], color='skyblue')
    plt.xlabel('Número de Gols', fontsize=12)
    plt.ylabel('Jogador', fontsize=12)
    plt.title('Top 10 Maiores Marcadores de Gols', fontsize=16)
    plt.tight_layout()

    # Passo 7: Exibir o gráfico no Streamlit
    st.pyplot(plt)

def exibir_valor_por_continente():
    st.title("Análise do Valor de Mercado por Continente")

    # Carregar os arquivos principais
    try:
        transferencias = pd.read_csv("transferencias_jogadores.csv")
        cruzamento = pd.read_excel("cruzamento.xlsx")
    except FileNotFoundError as e:
        st.error(f"Erro ao carregar os arquivos principais: {e}")
        return

    # Limpar os dados e normalizar a coluna de países
    cruzamento.columns = cruzamento.columns.str.strip()
    cruzamento["Country"] = cruzamento["Country"].str.strip().str.lower()

    # Carregar os arquivos dos continentes
    continentes = {
        "Europa": "mundo/europa.csv",
        "África": "mundo/africa.csv",
        "Ásia": "mundo/asia.csv",
        "América": "mundo/america.csv",
        "Oceania": "mundo/oceania.csv",
    }

    df_continentes_list = []
    for continente, path in continentes.items():
        try:
            df_continente = pd.read_csv(path, delimiter=",", on_bad_lines="skip", engine="python")
            df_continente["Continente"] = continente
            df_continentes_list.append(df_continente)
        except FileNotFoundError as e:
            st.error(f"Erro ao carregar arquivo de continente ({continente}): {e}")
            return
        except pd.errors.ParserError as e:
            st.error(f"Erro ao analisar o arquivo {path}: {e}")
            return

    # Concatenar os dados de continentes em um único DataFrame
    df_continentes = pd.concat(df_continentes_list, ignore_index=True)

    # Normalizar a coluna de países
    df_continentes["Country"] = df_continentes["Country"].str.strip().str.lower()

    # Realizar o merge entre transferências e cruzamento (associando os clubes aos países)
    transferencias = transferencias.merge(cruzamento, left_on="team", right_on="team", how="left")

    # Normalizar a coluna de países após o merge
    transferencias["Country"] = transferencias["Country"].str.strip().str.lower()

    # Realizar o merge entre transferências e o DataFrame de continentes
    transferencias = transferencias.merge(df_continentes, on="Country", how="left")

    # Calcular o valor de mercado por continente
    valor_por_continente = transferencias.groupby("Continente")["current_value"].sum()

    # Ordenar os valores de mercado por ordem decrescente
    valor_por_continente = valor_por_continente.sort_values(ascending=False)

    # Exibir a tabela de valores de mercado por continente
    st.write("### Valor de Mercado por Continente")
    st.dataframe(valor_por_continente)

    # Criar o gráfico de barras
    plt.figure(figsize=(12, 8))
    valor_por_continente.plot(kind="bar", color="skyblue", edgecolor="black")
    plt.title("Valor de Mercado Total por Continente", fontsize=16)
...
def pagina_inicial():
    st.title("Bem-vindo ao Dashboard de Transferências de Jogadores")

    # Explicação sobre o que o dashboard faz
    st.markdown("""  
    <div style="color: white; font-size: 25px; line-height: 1.6; 
                text-shadow: 2px 2px 4px #000000, -2px -2px 4px #000000, 
                            2px -2px 4px #000000, -2px 2px 4px #000000;">
        Este dashboard interativo foi desenvolvido para analisar e visualizar as transferências de jogadores
        de futebol ao redor do mundo. Ele permite que você explore dados detalhados sobre os valores de
        mercado das transferências, agrupados por continente, país, e clubes.
        <br><br>
        Com ele, você pode:
        <ul>
            <li>Ver o valor total das transferências de jogadores para diferentes continentes.</li>
            <li>Analisar os 10 maiores marcadores de transferências no mercado de futebol.</li>
            <li>Explorar as tabelas e gráficos que mostram as transferências e seu impacto no valor total do mercado de jogadores.</li>
            <li>Usar técnicas de Data Mining.</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)

    # Adicionar uma imagem de fundo maior e mais para a direita
    st.markdown(
        """
        <style>
        .stApp {
            background-image: url("https://img.freepik.com/vetores-premium/fundo-de-futebol-de-gradiente-dinamico_1226107-217.jpg?semt=ais_hybrid");
            background-size: 70%;  /* Aumentando o tamanho da imagem */
            background-position: 90% center;  /* Movendo a imagem mais para a direita */
...
if 'sidebar_option' not in st.session_state:
    st.session_state.sidebar_option = 'home'
...
if st.sidebar.button('10 melhores marcadores'):
    st.session_state.sidebar_option = '10 melhores marcadores'
    
if st.sidebar.button('valor dos continentes'):
    st.session_state.sidebar_option = 'valor dos continentes'
...
if st.session_state.sidebar_option == 'home':
    st.subheader("")
    pagina_inicial()
...
elif st.session_state.sidebar_option == '10 melhores marcadores':
    st.subheader("10 melhores marcadores")
    exibir_10_maiores_marcadores()

elif st.session_state.sidebar_option == 'valor dos continentes':
    st.subheader("valor dos continentes")
    exibir_valor_por_continente()
